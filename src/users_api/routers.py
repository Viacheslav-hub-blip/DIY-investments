...
from src.users_api.schemas.schemas import UserInsertDTO, UserLogin, TokenGetting
...
async def update_company(user_id: int, new_company: str,
                         user_service: Annotated[UserService, Depends(user_service)]
                         ):
    return user_service.update_liked_company(user_id, new_company)
